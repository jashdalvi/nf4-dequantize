...
def benchmark():
    ...
    for hd, m, seed, dt, label in CONFIGS:
        ...
        for name, fn in METHODS.items():
            ms_50, ms_20, ms_80 = triton.testing.do_bench(lambda: fn(layer),warmup=WARMUP,quantiles=QUANTILES,rep=REPS)
            results[label][name] = (ms_50, ms_20, ms_80)

    # Calculate median speedup
    for label, methods in results.items():
        unsloth, peft, custom_triton = methods["unsloth"], methods["peft"], methods["custom_triton"]
        print(f"Config: {label}")
        print(f"Custom triton: {custom_triton[0]:.4f} ms, {custom_triton[1]:.4f} ms, {custom_triton[2]:.4f} ms")
        print(f"Peft: {peft[0]:.4f} ms, {peft[1]:.4f} ms, {peft[2]:.4f} ms")
        print(f"Unsloth: {unsloth[0]:.4f} ms, {unsloth[1]:.4f} ms, {unsloth[2]:.4f} ms")
        print(f"Speed up: {unsloth[0] / custom_triton[0]:.4f}x vs unsloth, {peft[0] / custom_triton[0]:.4f}x vs peft")
