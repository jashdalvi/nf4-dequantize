...
def _dequantize_nf4_kernel(
    ...
    absmax_indices = tl.load(absmax_ptr  + absmax_offs,  mask=out_mask, other=0).to(tl.uint8)
...
def _dequantize_nf4(weight, quant_state):
    ...
    _dequantize_nf4_kernel[grid](
        weight, out, quant_state.absmax, quant_state.state2.absmax,
        quant_state.code, quant_state.state2.code, quant_state.offset,
        num_elements, BLOCK_SIZE=256,
    )
